...
?>>Сформировать</a>&nbsp;|&nbsp;
<?php if($_SESSION["group"]==2||$_SESSION["group"]==1||$_SESSION["group"]==4)echo '<a href="#" style="font-size:12px;" id="btnReport_cl_ex" onclick=\'if(document.getElementById("date_start_cl").value<=document.getElementById("date_end_cl").value){window.location.href="control/reports_cl_tr.php?mode=cl&mode_id="+$("input[name=report_cl]:checked").val()+"&cl_id="+$("#cl_id").val()+"&date_start_cl="+$("#date_start_cl").val()+"&date_end_cl="+$("#date_end_cl").val();} else {alert("Дата окончания периода должна быть позже даты начала!");}\'>Excel</a>';?>
