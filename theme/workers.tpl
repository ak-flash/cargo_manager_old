...
$('#btnPass').button();
$('#btnReport').button();$('#btnReport_new').button();
$('#btnMotive').button();
$('#btnBill').button();
$('#btnAdd').button();
$('#btnAll').button();
...
$('#btnShowDel').button();
    ...
    "openicon"  : "ui-icon-arrowreturn-1-e"
                  ...
                  subGridModel: [{ name : ['','Информация о сотруднике'], align:  ['center','left'], width : [1,640], params: ['id']} ],
rownumbers: false,
           pager: $('#tablePager'),
            ondblClickRow: function(id) {
       $("#fa_worker").load("theme/forms/add_worker.php?worker_id="+id); 
    $("#fa_worker").dialog({ title: 'Редактировать сотрудника №'+id },{
...
$("#fa_worker").load("theme/forms/add_worker.php"); 
    $("#fa_worker").dialog({ title: 'Новый сотрудник' },{
      width: 920,height: 730,
      modal: true,resizable: false
});
});
...
$("#fa_worker").load("theme/forms/add_motive.php"); 
    $("#fa_worker").dialog({ title: 'Схема рассчета комиссии' },{
...
jQuery('#table').trigger('reloadGrid');" style="width:100px;">Все</a>&nbsp;<a id="btnManagers" href="#" onclick="$('#table').setGridParam({url:'control/workers.php?mode=workers&m=managers'});
jQuery('#table').trigger('reloadGrid');">Менеджеры</a>&nbsp;<a class="button5" id="btnDrivers" href="#" onclick="$('#table').setGridParam({url:'control/workers.php?mode=workers&m=drivers'});
jQuery('#table').trigger('reloadGrid');">Водители</a>&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;<a class="button3" id="btnShowDel" href="#" onclick="$('#table').setGridParam({url:'control/workers.php?mode=workers&m=arhiv'});
...
<input type="button" id="btnBill" onclick='$("#fa_bill").load("theme/forms/add_zbill.php");$("#fa_bill").dialog({ title: "Ведомость по оплате заработной платы" },{width: 970,height: 710,modal: true,resizable: false});' value="Ведомость" style="font-size: 16px;width:150px;">&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;
 Зарплатная ведомость:&nbsp;&nbsp;&nbsp;&nbsp;<select name="zarplata_month" style="width:110px;" id="zarplata_month" class="select">
...
<option value="1">Январь</option>
  ...
  <option value="12">Декабрь</option></select>&nbsp;&nbsp;<select name="zarplata_year" style="width:110px;" id="zarplata_year" class="select">';
<?php 
$i=-1;
while ($i <= 1) {
...
echo '</select>&nbsp;&nbsp;&nbsp;<input type="button" id="btnReport_new" onclick=\'window.location.href="/control/zarplata_new.php?mode=zarplata&month="+$("#zarplata_month").val()+"&year="+$("#zarplata_year").val();\' value="Сформировать" style="font-size: 18px;width: 160px;">&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;';
