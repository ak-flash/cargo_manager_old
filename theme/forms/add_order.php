...
<script type="text/javascript">
    ...
    function Komiss() {
        ...
        if (document.getElementById('tr_cash_min').value == "") {
            tr_cash_min = 0;
        } else {
            var tr_cash_min = parseInt(document.getElementById('tr_cash_min').value, 10);
        }
        if (document.getElementById('tr_cash_plus').value == "") {
            tr_cash_plus = 0;
        } else {
            var tr_cash_plus = parseInt(document.getElementById('tr_cash_plus').value, 10);
        }
        ...
        cash = 0;
        if (cl_nds == 0 && tr_nds == 0) {
            var data = cl_cash_all - tr_cash_all;
        }
        if (cl_nds == 1 && tr_nds == 1) {
            var data = cl_cash_all - tr_cash_all;
        }
        if (cl_nds == 1 && tr_nds == 0) {
            var data = ((cl_cash_all - cl_cash_all /<?php echo $motive['motive_2'];?>) - tr_cash_all);
        }
        if (cl_nds == 1 && tr_nds == 2) {
            var data = (cl_cash_all - cl_cash_all *<?php echo $motive['motive_3'];?>) - tr_cash_all;
        }
        if (cl_nds == 0 && tr_nds == 1) {
            var data = (cl_cash_all - (tr_cash_all - tr_cash_all /<?php echo $motive['motive_4'];?>));
        }
        if (cl_nds == 0 && tr_nds == 2) {
            var data = (cl_cash_all - cl_cash_all *<?php echo $motive['motive_6'];?>) - tr_cash_all;
        }
        if (cl_nds == 2 && tr_nds == 1) {
            var data = cl_cash_all - tr_cash_all + tr_cash_all *<?php echo $motive['motive_7'];?>;
        }
        if (cl_nds == 2 && tr_nds == 0) {
            var data = cl_cash_all - tr_cash_all + tr_cash_all *<?php echo $motive['motive_8'];?>;
        }
        if (cl_nds == 2 && tr_nds == 2) {
            var data = cl_cash_all - tr_cash_all;
        }
    ...
    function SetVTL() {
        ...
        $("#car_select").load("control/auto/vtl_car.php?mode=auto_load"<?php if ($row['transp'] == 2) {
            echo ',function(data){$("#tr_autopark").val(' . $str_auto[0] . ').change();}';
        }?>);
        $("#dop_select").load("control/auto/vtl_car.php?mode=dop_load"<?php if ($row['transp'] == 2) {
            echo ',function(data){$("#dop_autopark").val(' . $str_auto[1] . ').change();}';
        }?>);
        $("#drv_select").load("control/auto/vtl_car.php?mode=drv_load"<?php if ($row['transp'] == 2) {
            echo ',function(data){$("#drv_autopark").val(' . $str_auto[2] . ').change();}';
        }?>);
...
//if(cl_cash_vtl>20000)$('#tr_cash').val(cl_cash_vtl-cl_cash_min+cl_cash_plus-5000);
        ...
        <?php if ($order != "") {
        echo '$("#tr_cash").val(' . $row['tr_cash'] . ');$("#vtl_cont_select").val(' . $row['cl_cont'] . ').change();';
    }?>
    ...
    $(document).ready(function () {
        ...
        <?php
        if ($order != "") {
            echo '$("#tr_manager").val(' . $row['tr_manager'] . ').change();';
        } else {
            echo '$("#tr_manager").val(' . $_SESSION["user_id"] . ').change();';
        }
...
//$('#tr_manager_name').html('&nbsp;&nbsp;Менеджер, предост. перевозчика: <font size="4"><b>'+arr[8]+'</b></font>');
    ...
    $("#form_order").submit(function () {
        ...
        validate = true;
        if ($("#client").val() == '') {
            $('#result_temp').append('Выберите клиента!<br>');
            validate = false;
        }

        if ($("#cl_cash").val() == '') {
            $('#result_temp').append('Заполните ставку клиента!<br>');
            validate = false;
        }


        if ($("#in_adr").val() == '') {
            $('#result_temp').append('Выберите адрес загрузки!<br>');
            validate = false;
        }

        if ($("#out_adr").val() == '') {
            $('#result_temp').append('Выберите адрес выгрузки!<br>');
            validate = false;
        }

        if ($("#transporter").val() == '') {
            $('#result_temp').append('Выберите перевозчика!<br>');
            validate = false;
        }

        if ($("#tr_receive").val() == '0') {
            $('#result_temp').append('Выберите получателя для перевозчика!<br>');
            validate = false;
        }

        if ($("#cl_event").val() == '0') {
            $('#result_temp').append('Укажите точку оплаты для клиента!<br>');
            validate = false;
        }

        if ($("#tr_event").val() == '0') {
            $('#result_temp').append('Укажите точку оплаты для перевозчика!<br>');
            validate = false;
        }

        if ($("#tr_cash").val() == '') {
            $('#result_temp').append('Заполните ставку перевозчика!<br>');
            validate = false;
        }

        if ($("#out_data1").val() == '') {
            $('#result_temp').append('Заполните дату выгрузки!<br>');
            validate = false;
        }
        ...
        if ($(':radio[name=transp_mode]').filter(':checked').val() == 'vtl' && ($("select#tr_autopark").val() == 0 || $("select#dop_autopark").val() == 0 || $("select#drv_autopark").val() == 0)) {
            $('#result_temp').append('Выберите тягач, полуприцеп и водителя!<br>');
            validate = false;
        }


        if (!validate) {
            $("#result_temp").dialog({title: 'Внимание'}, {width: 330}, {modal: true}, {resizable: false}, {
                buttons: {
                    "Ok": function () {
                        $(this).dialog("close");
                    }
...
//
        ...
        if ($row['cl_plus'] != 0) print('$("#cl_cash_plus").val(' . $row['cl_plus'] . ');');
        if ($row['tr_minus'] != 0) print('$("#tr_cash_min").val(' . $row['tr_minus'] . ');');
        if ($row['tr_plus'] != 0) print('$("#tr_cash_plus").val(' . $row['tr_plus'] . ');');

        if ($row['cl_rashod_na_cl'] != 0) print('$("#cl_rashod_na_cl").val(' . $row['cl_rashod_na_cl'] . ');');
        if ($row['cl_rashod_sb'] != 0) print('$("#cl_rashod_sb").val(' . $row['cl_rashod_sb'] . ');');
        if ($row['cl_komissia'] != 0) print('$("#cl_komissia").val(' . $row['cl_komissia'] . ');');
        if ($row['tr_komissia'] != 0) print('$("#tr_komissia").val(' . $row['tr_komissia'] . ');');
        ...
        if ($row['transp'] == 2 || $row['transp'] == 462) {
            echo 'SetVTL();$("#km").val(' . $row['km'] . ');$("#tr_sel_vtl").val(' . $row['transp'] . ').change;$("#tr_sel_r_vtl").val(' . $row['tr_receive'] . ').change;';
        } else {
...
<form method="post" id="form_order">
    ...
    <div id="hidden_value_tr_receive" style="display: none;"><input type="hidden" name="tr_receive" id="tr_receive"
                                                                    value="0"></div>
    ...
    <div id="order_tabs">
        ...
        <div id="order_tabs-1"
             style="height: 31em; overflow: auto;<?php if ($_GET['gruz_id'] != '') echo 'display:none;'; ?>">
            ...
            <table>
                <tr>
                    <td width="120">
                        <b>Клиент:</b>
                        <div id="cl_pref_temp" style="display:inline;">
                            <div>
                    </td>
                    <td width="310">
                    ...
                    <td>
                        ...
                        <a href="#" style="font-size: 20px;text-decoration: none;float:left;"
                           onclick='$("#fa_cl").load("theme/forms/add_client.php");$("#fa_cl").dialog({ title: "Новый клиент" },{width: 980,height: 560,modal: true,resizable: false});'>&nbsp;&nbsp;<img
                                    src="data/img/plus.png"></a>
...
$("#fa_cl").dialog({ title: "Редактировать клиента №"+$("#client").val() },{width: 980,height: 560,modal: true,resizable: false});} else {alert("Выберите клиента!");}'>&nbsp;&nbsp;<img
                                    src="data/img/pencil.png">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a>
                        ...
                        <select name="cl_nds" id="cl_nds" class="select">
                            ...
                            <option value="2" onclick="Komiss();">НАЛ</option>
                        </select>
                    </td>
                </tr>
                <tr>
            ...
            <fieldset>
                ...
                <table>
                    <tr>
                        <td align="right">
                            <b>Ставка:</b></td>
                        <td width="200"><input name="cl_cash" id="cl_cash" style="width: 70px;" placeholder="0"
                                               class="input" onchange="Komiss();">
                            ...
                            <select name="cl_currency" style="width:70px;" id="cl_currency" class="select"
                                    onchange="$('#currency_info1').html($(this).val());$('#currency_info2').html($(this).val());$('#currency_info3').html($(this).val());$('#currency_info4').html($(this).val());$('#currency_info5').html($(this).val());">
                        ...
                        <td width="110"><b>Срок оплаты:</b>&nbsp;</td>
                        <td width="110"><input name="cl_tfpay" id="cl_tfpay" style="width: 50px;" placeholder="0"
                                               class="input" value="">&nbsp;дней&nbsp;
                        </td>

                        <td rowspan="2"><b>Точка отсчета:</b>&nbsp;&nbsp;<select name="cl_event" style="width:180px;"
                                                                                 id="cl_event" class="select">
                    ...
                    <tr>
                        ...
                        <td><br><input name="cl_cash_plus" id="cl_cash_plus" style="width: 60px;" placeholder="0"
                                       class="input" onchange="Komiss()"></td>

                        <td align="right"><br>
                            <b>Комиссия</b></td>
                        <td><br><input name="cl_komissia" id="cl_komissia" style="width: 40px;" placeholder="0"
                                       class="input" onchange=""> %
                        </td>
                    </tr>
                </table>
            ...
            <fieldset>
                <legend><b>Дополнительные расходы:</b></legend>
                <table>
                    <tr>
                        ...
                        <td>
                            <input name="cl_cash_min" id="cl_cash_min" style="width: 60px;" placeholder="0"
                                   class="input" onchange="Komiss()">
                            <div id="currency_info1" style="display: inline;"></div>
                        </td>
                        ...
                        <td>
                            <input name="cl_rashod_sb" id="cl_rashod_sb" style="width: 60px;" placeholder="0"
                                   class="input" onchange="Komiss()">
                            <div id="currency_info2" style="display: inline;"></div>
                        </td>

                        <td align="right" width="250">
                            Доп. Расходы на клиента:
                        </td>
                        <td>
                            <input name="cl_rashod_na_cl" id="cl_rashod_na_cl" style="width: 60px;" placeholder="0"
                                   class="input" onchange="Komiss()">
                            <div id="currency_info3" style="display: inline;"></div>
                        </td>
                    </tr>
                </table>
        ...
        <div id="order_tabs-3" style="height: 31em; overflow: auto">
            ...
            <div id="in_adr_select">&nbsp;&nbsp;&nbsp;&nbsp;<a id="btnAdd_adress_in" class="btnAdress"
                                                               href="#">новый</a></div>
            <br>
            ...
            <b>Адреса выгрузки:</b>
            <hr style="width: 100%; height: 2px;"/>
            <div id="out_adr_select">&nbsp;&nbsp;&nbsp;&nbsp;<a id="btnAdd_adress_out" href="#"
                                                                class="btnAdress">новый</a></div>
            <br>
        ...
        <div id="order_tabs-4" style="height: 31em; overflow: auto;">
            ...
            <fieldset>
                <legend><b>Груз:</b></legend>
                <table cellpadding="5">
                    <tr>
                        <td>Наименование:</td>
                        <td>
                            <input name="gruz_name" id="gruz_name" style="width: 140px;" placeholder="Укажите название"
                                   class="input">
                        </td>
                        <td>Вес:</td>
                        <td>
            ...
            <fieldset>
                <legend><b>Грузчики:</b></legend>
                <table cellpadding="5">
                    <tr>
                        <td>Стоимость грузчиков:</td>
                        <td>
                            <input name="tr_gruz_worker" id="tr_gruz_worker" style="width:50px;" placeholder="0"
                                   class="input">
            ...
            <fieldset>
                <legend><b>Требования к машине (Особые условия):</b></legend>
...
<?php if ($row['car_notify'] != "") echo $row['car_notify']; else echo 'Чистый кузов, возможность пломбировки, БЕЗ догрузов, Водитель гражданин РФ паспорт с собой, без судимости'; ?>
        ...
        <div id="order_tabs-2" style="height: 31em; overflow: auto">
            ...
            <input type="radio" name="transp_mode" id="tr_mode"
                   value="tr" <?php if ($row['transp'] != 2 || $row['transp'] != 462)
                echo 'checked'; ?> onclick="UnSetVTL();">&nbsp;&nbsp;<b>Перевозчик:</b>

            <input type="radio" name="transp_mode" id="vtl_mode"
                   value="vtl" <?php if ($row['transp'] == 2 || $row['transp'] == 462)
            ...
            <table height="80">
                <tr>
                    <td width="100">
                        ...
                        <div id="tr_pref_temp" style="display:inline;">
                            <div>

                    </td>
                    <td width="250">
                        <div id="tr_select" style="float:left; width:255px;"></div>
                        <div id="tr_select_vtl" style="display:none;"></div>
                    ...
                    <td width="100">
                        ...
                        <div id="tr_edit_vtl" style="display:inline;">
                            ...
                            <a href="#" style="font-size: 20px;text-decoration: none;float:left;"
                               onclick='$("#fa_tr").load("theme/forms/add_transporter.php");$("#fa_tr").dialog({ title: "Новый перевозчик" },{width: 990,height: 560,modal: true,resizable: false});'>&nbsp;&nbsp;<img
                                        src="data/img/plus.png"></a>
                            <a href="#" style="font-size: 20px;text-decoration: none;float:left;"
                               onclick='if($("#transporter").val()!=""){$("#fa_tr").load("theme/forms/add_transporter.php?tr="+$("#transporter").val());$("#fa_tr").dialog({ title: "Редактировать перевозчика №"+$("#transporter").val()},{width: 990,height: 560,modal: true,resizable: false});} else {alert("Выберите Перевозчика!");}'>&nbsp;&nbsp;<img
                                        src="data/img/pencil.png">&nbsp;&nbsp;</a>
                        </div>
                    ...
                    <td align="right">&nbsp;&nbsp;&nbsp;<b>Транспорт</b></td>
                    <td>

                        <div id="car_select" style="display:inline;">
                            <select name="tr_autopark" style="width:130px;" id="tr_autopark" class="select">
                                <option value="0">Выберите...</option>
                            </select></div>
                    </td>
                    <td align="left" valign="middle">&nbsp;&nbsp;<div id="car_add" style="display:inline;"><img id="img"
                                                                                                                src="data/img/car_add.png"
                                                                                                                style="cursor: pointer;margin-top:5px;"
                                                                                                                onclick='if($("#tr_autopark").val()==0){$("#fa_car").load("theme/forms/add_car.php?tr="+$("#transporter").val());$("#fa_car").dialog({ title: "Новый транспорт" },{width: 580,height: 740,modal: true,resizable: false});} else {$("#fa_car").load("theme/forms/add_car.php?tr="+$("#transporter").val()+"&mode=car&edit=1&car_id="+$("#tr_autopark").val()); $("#fa_car").dialog({ title: "Редактировать транспорт" },{width: 580,height: 790,modal: true,resizable: false});}'>&nbsp;&nbsp;<a
                                    href="#"
                                    style="font-size: 20px;color:black;text-decoration: none;float:right;display:inline;margin-top:5px;"
                                    onclick='$("#fa_show_car").load("show_car.php?tr="+$("#transporter").val());$("#fa_show_car").dialog({ title: "Автопарк" },{width: 880,height: 560,modal: true,resizable: false});'>[Все]</a>
                        </div>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        <div id="tr_receive_select"></div>
                    </td>

                    <td>

                        <div align="right" id="tr_days_input" style="display:none;"><b>Простой</b>:&nbsp;<input
                                    name="tr_days" id="tr_days" style="width: 30px;" placeholder="0" class="input"
                                    value="0"> сут.
                        </div>

                    </td>
                    <td>
                        <div id="drv_select" style="display:inline;"></div>
                    <td>
                        <div id="dop_select" style="display:inline;"></div>
                    </td>
            ...
            <fieldset>
                ...
                <table cellspacing="5">
                    <tr>
                        <td width="90">
                            <b>Ставка:</b></td>
                        <td width="160"><input name="tr_cash" id="tr_cash" style="width: 70px;" placeholder="0"
                                               class="input" onchange="Komiss()">
                        ...
                        <td>&nbsp;&nbsp;&nbsp;Способ опл.
                            <div id="vtl_nds" style="display:none;"></div>
                        </td>
                        <td align="left">
                            <div id="tr_nds_temp" style="display:inline;"><select name="tr_nds" id="tr_nds"
                                                                                  class="select">
...
">с НДС
                        ...
                        <td colspan="4"><b>Точка отсчета:</b>&nbsp;&nbsp;<select name="tr_event" style="width:200px;"
                                                                                 id="tr_event" class="select">
                                <option value="0">Выберите...</option>
                                <option value="1">Загрузка</option>
                                <option value="2">Выгрузка</option>
                                <option value="3">Поступление факсимильных документов</option>
                    ...
                    <tr>
                        <td>
                            <div id="show_tr_dpay1"><b>Срок опл.:</b>&nbsp;</div>
                        </td>
                        <td width="130">
                            <div id="show_tr_dpay2"><input name="tr_tfpay" class="input" id="tr_tfpay"
                                                           style="width: 30px;" placeholder="0">&nbsp;дн&nbsp;<input
                                        type="checkbox" name="days_tfpay" id="days_tfpay" value="1">Банк.
                            </div>
                        </td>
                        ...
                        <td><input name="tr_cash_min" id="tr_cash_min" style="width: 60px;" placeholder="0"
                                   class="input" onchange="Komiss()"></td>
                        <td align="right">
                            Надбавка(-):
                        </td>
                        <td><input name="tr_cash_plus" id="tr_cash_plus" style="width: 60px;" placeholder="0"
                                   class="input" onchange="Komiss()"></td>
                    ...
                    <tr>
                        ...
                        <td align="right">
                            <b>Комиссия</b></td>
                        <td><input name="tr_komissia" id="tr_komissia" style="width: 40px;" placeholder="0"
                                   class="input" onchange=""> %
                        </td>
                    </tr>
                    </tr>
        ...
        <div id="order_tabs-5" style="height: 31em; overflow: auto">
            ...
            <fieldset>
                <legend>Дополнения к заявке:</legend>
                <b>Клиенту</b><br><textarea style="width:98%;" rows="3"
                                            name="print_cl"><?php echo $row['print_cl']; ?></textarea><br><b>Перевозчику</b><br><textarea
                        style="width:98%;" rows="3" name="print_tr"><?php echo $row['print_tr']; ?></textarea>
            </fieldset>
            <fieldset>
                <legend>Примечание к заявке:</legend>
                <textarea rows="8" name="order_notify"
                          style="width:98%;"><?php echo addslashes($row['notify']); ?></textarea>
            </fieldset>
        </div>
...
<b>Менеджер заявки:</b>&nbsp;&nbsp;</td><td><select name="select_manager" id="select_manager" style="width:174px;" class="select">';
            ...
            <fieldset>
                ...
                <table>
                    <tr>
                        ...
                        <td>
                            <input name="in_time11" id="in_time11" style="width: 38px;" placeholder="-" class="input">&nbsp;&nbsp;по
                            Время:&nbsp;&nbsp;<input name="in_time12" id="in_time12" style="width: 38px;"
                                                     placeholder="-" class="input">&nbsp;&nbsp;&nbsp;*формат: дд/мм/гггг
                        </td>
                    </tr>
                    <tr>
                        <td align="right" width="100">
                        ...
                        <td>
                            <input name="in_time21" id="in_time21" style="width: 38px;" placeholder="-" class="input">&nbsp;&nbsp;по
                            Время:&nbsp;&nbsp;<input name="in_time22" id="in_time22" style="width: 38px;"
                                                     placeholder="-" class="input">&nbsp;&nbsp;&nbsp;*формат: дд/мм/гггг
                        </td>
                    </tr>
                </table>
            ...
            <fieldset>
                ...
                <table>
                    <tr>
                        ...
                        <td>
                            <input name="out_data1" id="out_data1" style="width: 80px;" placeholder="-" class="input"
                                   value="">
                        </td>
                        <td align="right" width="80">
                            Время:&nbsp;&nbsp;
                        ...
                        <td>
                            <input name="out_time1" id="out_time1" style="width: 38px;" placeholder="-" class="input">&nbsp;&nbsp;&nbsp;*формат:
                            дд/мм/гггг
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <input name="out_data2" id="out_data2" style="width: 80px;" placeholder="-" class="input"
                                   value="">
                        </td>
                        <td align="right" width="80">
                            Время:&nbsp;&nbsp;
                        ...
                        <td>
                            <input name="out_time2" id="out_time2" style="width: 38px;" placeholder="-" class="input">&nbsp;&nbsp;&nbsp;*формат:
                            дд/мм/гггг
                        </td>
                    </tr>
                </table>
    ...
    <div id="security_code" style="float:left;">&nbsp;&nbsp;&nbsp;Пароль: <input type="password" name="order_code"
                                                                                 id="order_code" style="width: 140px;"
                                                                                 value="" class="input"></div>
    <div id="block_info" style="float:right;font-size: 24px;color:red;"></div>
