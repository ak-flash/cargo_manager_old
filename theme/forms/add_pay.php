...
function Komiss() {
    $('#check_info').html('<table bgcolor="#FFFFFF" cellspacing="10"><tr><td width="220">Планируемая сумма:&nbsp;</td><td width="200">Предлагается:&nbsp;</td></tr><tr><td><b><font size="5"><div id="plan_cash" style="float:left;margin-left:20px;">0</div></font></b>&nbsp;</td><td><font size="5"><div id="inc" style="float:left;"></div> <div id="plan_pay" style="float:left;">0</div></font></td></tr></table>');

    $('#cash').val($('#cash').val().replace(/,+/, "."));
    $('#plan_pay').html(number_format($('#cash').val(), 0, '.', ' '));

    if ($('#order').val() == '') $('#order_info').html('');
    if ($('#category').val() == '2') {
        $('#order_info').html('');
        $('#plan_cash').html(number_format($('#cash').val(), 2, '.', ' '));
        $('#order').val('')
    }

    if ($('#order').val() != '' && $('#category').val() != '2') {
        $('#order_info').html('&nbsp;<u>по заявке №' + $('#order').val() + '</u>&nbsp;');
        ...
        $("#plan_cash").load('control/pay_load.php?order_id=' + $('#order').val() + '&order_type=' + $('#order_type').val() + '&mode=' + $('#appointment').val() + '&way=' + $('#way').val(), function (data) {
            ...
            if (arr[0] == 'not_exist') {
                $("#plan_cash").html("0");
                $('#error').val("1");
                toastr.error('Заявки не существует!');
            }
            if (arr[0] == 'success') {
                $("#plan_cash").html(number_format(arr[1], 0, '.', ' ') + ' <small>' + arr[4] + '</small>');
                $('#plan_cash_int').val(arr[1]);
                $('#cash').val(arr[1]);
                $('#plan_pay').html(arr[1]);
                $('#error').val("0");
            }
            if (arr[0] == 'fully_payed') {
                toastr.info('Заявка полностью оплачена!');
                $('#check_info').html('<input type="hidden" name="pay_cash_full" id="pay_cash_full" value="1"><table bgcolor="#FFFFFF" cellspacing="10" height="80" width="450" ><tr><td align="center"><font color="green" size="6">' + arr[1] + '</font></td></tr></table>');
            }
        });
    }
}
...
<input type="hidden" name="add_name" id="add_name" value="<?php echo $_SESSION['user_id'];?>">
    ...
    <fieldset style="width:93%;margin-top:0;">
        ...
        <table>
            <tr>
                <td>
                    <b><font size="4">№<?php echo $row['id']; ?></font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php if ($pay_id != '') {
                            switch ($row['status']) {
                                case '0':
                                    $status = '<font size="4" color="#FF4F4F">Не проведен</font>';
                                    break;
                                case '1':
                                    $status = '<font size="4" color="#6AAF3B">Проведен</font>';
                                    break;
                            }
                            echo $status;
                        } ?></b></td>
                <td align="right" width="100">Дата:</td>
                <td width="150"><input type="text" id="date_pay" name="date_pay" style="width:80px;"
                                       value="<?php if ($pay_id == '') {
                                           echo date('d/m/Y');
                                       } else {
                                           echo date('d/m/Y', strtotime($row['date']));
                                       } ?>" class="input"></td>
                <td><input type="checkbox" name="transaction" id="transaction" <?php if ($row['status'] == '1') {
                        echo 'checked disabled';
                    } ?>
                           onclick="if(this.checked){ $(this).val(1);$('#prov').html('<u>ПРОВЕДЕНИЯ</u>'); } else { $(this).val(0);$('#prov').html('<u>ПЛАНИРОВАНИЯ</u>')}"
                           value="<?= $row['status'] ?>">&nbsp;&nbsp;<font size="3">Провести</font></td>
            </tr>
        </table>
    </fieldset>
    <fieldset style="margin-top:5px;width:93%;">
        <table cellspacing="5">
            <tr>
                <td align="right" width="100">Категория</td>
                <td><select name="category" id="category" class="select"
                            onchange="Komiss();$('#way').val(0).change();$('#appointment').val(0).change();$('#appoint').html('')">
                        <option value="1" onclick='$("#order").attr("disabled","");'>Основная</option>
                        <option value="2" onclick='$("#order").attr("disabled","disabled");'>Дополнительная</option>
                    </select></td>
                <td align="right" width="100"><b>Источник получатель</b></td>
                <td>
                    <table cellspacing="5">
                        <tr>
                            <td><select name="payment_source" id="payment_source"
                                        class="select" <?php if ($row['status'] != '1') {
                                    echo 'onchange="this.options[this.selectedIndex].onclick();"';
  ...
  <option value="1">поступление</option>
    ...
    <td colspan="2" rowspan="3">
        ...
        <div id="bill_info_balance" style="margin-left:20px;"></div>
            ...
            <tr>
                <td align="right">Способ платежа</td>
                <td><select name="nds" id="nds" class="select">
                        <option value="33">...</option>
                        <option value="0"
                                onclick='$("#payment_source").val(3).change();$("#bill_info").html("");$("#bill_info_balance").html("");$("#bill_info_bank").html("");$("#payment_source [value=1]").attr("disabled","disabled");$("#payment_source [value=2]").attr("disabled","");$("#payment_source [value=3]").attr("disabled","");$("#payment_source [value=4]").attr("disabled","");$("#payment_source [value=5]").attr("disabled","");'>
                            без НДС
                        </option>
                        <option value="1"
                                onclick='$("#payment_source").val(2).change();$("#bill_info").html("");$("#bill_info_balance").html("");$("#bill_info_bank").html("");$("#payment_source [value=1]").attr("disabled","disabled");$("#payment_source [value=2]").attr("disabled","");$("#payment_source [value=3]").attr("disabled","");$("#payment_source [value=4]").attr("disabled","");$("#payment_source [value=5]").attr("disabled","");'>
                            с НДС
                        </option>
                        <option value="2"
                                onclick='$("#payment_source").val(1).change();$("#pay_bill").val("1");$("#bill_info").html("<b>Наличные</b>");$("#bill_info_bank").html("");$("#bill_info_balance").html("<font size=4>Баланс: <b><?php echo number_format($bill_info[0] / 100, 2, ',', ' '); ?></b></font>");$("#payment_source [value=1]").attr("disabled","");$("#payment_source [value=2]").attr("disabled","disabled");$("#payment_source [value=3]").attr("disabled","disabled");$("#payment_source [value=4]").attr("disabled","disabled");$("#payment_source [value=5]").attr("disabled","disabled");'>
                            НАЛ
                        </option>
                    </select></td>
            ...
            <tr>
                <td align="right"><b>Назначение</b></td>
                <td>
            ...
            <tr>
                <td align="right"><b>Заявка</b>&nbsp;&nbsp;</td>
                <td><input type="number" type="text" id="order" name="order" type="number"
                           style="width:80px;display:inline;" value="" onchange="Komiss()" placeholder="-"
                           class="input"/>&nbsp;&nbsp;

                    <a href="javascript:" onclick="if($('#order').val()!=''&&$('#category').val()!='2'){
Komiss() }"><img src="data/img/arrow-circle-double.png"></a></td>
                <td align="right"><font size="4"><b>Сумма</b></font></td>
                <td><input type="text" id="cash" name="cash" style="width:100px;" type="number" value=""
                           onchange="$('#plan_pay').html($(this).val());" placeholder="0" class="input"/>
...
<br>
<div align="center" style="float:left;margin-left:120px;margin-top:-10px;"><input type="submit" id="save_pay" value="<?php if($pay_id!=''){echo 'Сохранить';} else {echo 'Создать';}?>"
                                                                                  style="width: 250px;">
    <input type="button" id="btnClose_pay" onclick="$('#fa_pay').dialog('close');" value="Отмена" style="width: 160px;">
</div>
    <fieldset style="width:95%;margin-top:40px;">
        <legend>Информация:</legend>
        <div style="height: 200px; margin-left: 15px; overflow: auto">
            <table cellspacing="5">
                <tr>
                    <td><b>Данный платёж создаётся для:</b><br>
                        <b>
                            <div id="prov" style="float:left;"><u>ПЛАНИРОВАНИЯ</u></div>
                        </b>
                        <div style="float:left;">&nbsp;оплаты&nbsp;</div>
                        <b>
                            <div id="appoint" style="float:left;"></div>
                        </b>
                        <div id="order_info" style="float:left;"></div>
                        <br>
                        <hr>
                        <div id="check_info">
                            <table bgcolor="#FFFFFF" cellspacing="10">
                                <tr>
                                    <td width="260">Планируемая сумма:&nbsp;</td>
                                    <td width="260">Предлагается:&nbsp;</td>
                                </tr>
                                <tr>
                                    <td><b><font size="5">
                                                <div id="plan_cash" style="float:left;margin-left:20px;">0</div>
                                            </font></b></td>
                                    <td><font size="5">
                                            <div id="inc" style="float:left;"></div>
                                            <div id="plan_pay" style="float:left;">0</div>
                                        </font></td>
                                </tr>
                            </table>
                        </div>
