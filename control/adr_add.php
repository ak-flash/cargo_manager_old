...
if(@$_POST['country']==""){echo $error.'"страна"'.$err; $validate=false;}
if($_SESSION["group"]!=4){if(@$_POST['obl']==""&&@$_POST['city']!="Москва"&&@$_POST['city']!="Санкт-Петербург"){echo $error.'"область"'.$err; $validate=false;}
if(@$_POST['city']==""){echo $error.'"город"'.$err; $validate=false;}
if(@$_POST['street']==""){echo $error.'"улица"'.$err; $validate=false;}
if(@$_POST['ttn']!="on"&&@$_POST['dom']==""){echo $error.'"дом"'.$err; $validate=false;} 
//if(@$_POST['flat']==""||!intval($_POST['flat'])){echo $error.'"квартира(офис)"'.$err; $validate=false;} 
if(@$_POST['adr_mode']==""||!intval($_POST['adr_mode'])){echo $error.'"вид адреса"'.$err; $validate=false;} 
//if(@$_POST['contact_name']==""){echo $error.'"контактное лицо"<br>';}
//if(@$_POST['contact_phone']==""){echo $error.'"номер телефона контактного лица"<br>';}
}

if(@$_POST['city']!=""&&@$_POST['obl']!=""&&@$_POST['street']!=""&&@$_POST['dom']!=""&&@$_POST['edit']!="1"){
$query_adr = "SELECT `Id` FROM `adress` WHERE `obl` LIKE '%".mysql_real_escape_string(@$_POST['obl'])."' AND `city` LIKE '%".mysql_real_escape_string(@$_POST['city'])."' AND `street` LIKE '%".mysql_real_escape_string(@$_POST['street'])."' AND `dom` LIKE '".mysql_real_escape_string(@$_POST['dom'])."' AND `adr_mode` LIKE '%".mysql_real_escape_string(@$_POST['adr_mode'])."' AND `postcode`='".mysql_real_escape_string(@$_POST['postcode'])."'";
$result_adr = mysql_query($query_adr) or die(mysql_error());
if (mysql_num_rows($result_adr)>0){
echo $error.'"<font size=3>Такой Адрес уже существует!</font>"'.$err; $validate=false;
}
}
...
$obl=mysql_real_escape_string(stripslashes($_POST['obl']));
$city=mysql_real_escape_string(stripslashes($_POST['city']));
$street=mysql_real_escape_string(stripslashes($_POST['street']));
$dom=mysql_real_escape_string(stripslashes($_POST['dom']));
$dom_extra=mysql_real_escape_string(stripslashes($_POST['dom_extra']));
...
if($adr_mode==3||$adr_mode==4||$adr_mode==5){$select=$postcode.' ';}
