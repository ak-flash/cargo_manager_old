...
function komissia_adidas($cl_cash,$cl_minus,$cl_plus,$cl_nds_inp,$tr_cash,$tr_minus,$tr_plus,$tr_nds_inp){
$cl_cash_all=0;
$tr_cash_all=0;
$cash=0;
...
//if($cl_nds==1&&$tr_nds==0)$cash=$cl_cash_all-($tr_cash_all+$tr_cash_all*$motive['motive_2']);
//if($cl_nds==1&&$tr_nds==2)$cash=$cl_cash_all-$cl_cash_all*$motive['motive_3']-$tr_cash_all;
//if($cl_nds==0&&$tr_nds==1)$cash=$cl_cash_all-$tr_cash_all+$tr_cash_all*$motive['motive_4'];
...
$i=0;
$query = "SELECT `id`,`name`,`group`,`zarplata`,`motive`,`ndfl`,`cl_way` FROM `workers` WHERE `delete`='0' AND `group`!='5' AND `group`!='2' AND `group`!='6' ORDER BY `Id` ASC";
...
$total_info_adidas=0;
$total_info_adidas_pay=0;
$total_info_adidas_pay_last1=0;
$total_info_adidas_pay_last2=0;

$total_info_tr_adidas_pay=0;
$total_info_tr_adidas_pay_last1=0;
$total_info_tr_adidas_pay_last2=0;

$total_info_tr_dop_adidas_pay=0;
$total_info_tr_dop_adidas_pay_last1=0;
$total_info_tr_dop_adidas_pay_last2=0;
...
if($row_t['client']==651) $cash=komissia_adidas($row_t['cl_cash'],$row_t['cl_minus'],$row_t['cl_plus'],$row_t['cl_nds'],$row_t['tr_cash'],$row_t['tr_minus'],$row_t['tr_plus'],$row_t['tr_nds']); else $cash=komissia($row_t['cl_cash'],$row_t['cl_minus'],$row_t['cl_plus'],$row_t['cl_nds'],$row_t['tr_cash'],$row_t['tr_minus'],$row_t['tr_plus'],$row_t['tr_nds']);

if($row_t['client']==651) $total_info_adidas=(int)$total_info_adidas+(int)$cash;
...
if($row_t['client']==651) $total_info_adidas_pay=(int)$total_info_adidas_pay+(int)$cash;
}
...
$t_order_last1[$row_t['id']]=$cash;$total_last1=(int)$total_last1+(int)$cash;

if($row_t['client']==651) $total_info_adidas_pay_last1=(int)$total_info_adidas_pay_last1+(int)$cash;
...
$t_order_last2[$row_t['id']]=$cash;$total_last2=(int)$total_last2+(int)$cash;

if($row_t['client']==651) $total_info_adidas_pay_last2=(int)$total_info_adidas_pay_last2+(int)$cash;
...
if($row_tr['client']==651) $cash=komissia_adidas($row_tr['cl_cash'],$row_tr['cl_minus'],$row_tr['cl_plus'],$row_tr['cl_nds'],$row_tr['tr_cash'],$row_tr['tr_minus'],$row_tr['tr_plus'],$row_tr['tr_nds']); else $cash=komissia($row_tr['cl_cash'],$row_tr['cl_minus'],$row_tr['cl_plus'],$row_tr['cl_nds'],$row_tr['tr_cash'],$row_tr['tr_minus'],$row_tr['tr_plus'],$row_tr['tr_nds']);
...
if($row['motive']==1){$tr_order[$row_tr['id']]=$cash;$total_tr=(int)$total_tr+(int)$cash;}
if($row['motive']==3){
if($row_tr['client']==$row['cl_way']){$tr_order_way[$row_tr['id']]=(int)$cash;$total_tr_way=(int)$total_tr_way+(int)$cash;} else {$tr_order[$row_tr['id']]=$cash;$total_tr=(int)$total_tr+(int)$cash;}
}

if($row_tr['client']==651) $total_info_tr_adidas_pay=(int)$total_info_tr_adidas_pay+(int)$cash;
...
if($row['motive']==1){$tr_order_last1[$row_tr['id']]=(int)$cash/2;$tr_last1=(int)$tr_last1+(int)$cash;}
if($row['motive']==3){
if($row_tr['client']==$row['cl_way']){$tr_order_way_last1[$row_tr['id']]=(int)$cash/2;$tr_way_last1=(int)$tr_way_last1+(int)$cash;} else {$tr_order_last1[$row_tr['id']]=(int)$cash/2;$tr_last1=(int)$tr_last1+(int)$cash;}
}

if($row_tr['client']==651) $total_info_tr_adidas_pay_last1=(int)$total_info_tr_adidas_pay_last1+(int)$cash;
...
if($row['motive']==1){$tr_order_last2[$row_tr['id']]=(int)$cash/2;$tr_last2=(int)$tr_last2+(int)$cash;}
if($row['motive']==3){
if($row_tr['client']==$row['cl_way']){$tr_order_way_last2[$row_tr['id']]=(int)$cash;$tr_way_last2=(int)$tr_way_last2+(int)$cash;} else {$tr_order_last2[$row_tr['id']]=(int)$cash/2;$tr_last2=(int)$tr_last2+(int)$cash;}
}

if($row_tr['client']==651) $total_info_tr_adidas_pay_last2=(int)$total_info_tr_adidas_pay_last2+(int)$cash;
...
$s++;$tr_dop_order[$row_tr_dop['id']]=(int)$cash/2;$total_tr_dop=(int)$total_tr_dop+(int)$cash;}

if($row_tr_dop['client']==651) $total_info_tr_dop_adidas_pay=(int)$total_info_tr_dop_adidas_pay+(int)$cash;
...
if((int)($pays_tr_dop/100)==(int)$row_tr_dop['cl_cash']){
$s++;$tr_dop_order_last1[$row_tr_dop['id']]=(int)$cash/2;$tr_dop_last1=(int)$tr_dop_last1+(int)$cash;}

if($row_tr_dop['client']==651) $total_info_tr_dop_adidas_pay_last1=(int)$total_info_tr_dop_adidas_pay_last1+(int)$cash;
...
if((int)($pays_tr_dop/100)==(int)$row_tr_dop['cl_cash']){
$s++;$tr_dop_order_last2[$row_tr_dop['id']]=(int)$cash/2;$tr_dop_last2=(int)$tr_dop_last2+(int)$cash;}


if($row_tr_dop['client']==651) $total_info_tr_dop_adidas_pay_last2=(int)$total_info_tr_dop_adidas_pay_last2+(int)$cash;
...
$zarp=((int)$total+((int)$total_tr/2)+((int)$total_tr_dop/2)-((int)$total_info_adidas_pay+((int)$total_info_tr_adidas_pay/2)+((int)$total_info_tr_dop_adidas_pay/2)))*0.1;$proc=" (10%)";$pr=0.1;
$zarp_adidas=((int)$total_info_adidas_pay+((int)$total_info_tr_adidas_pay/2+((int)$total_info_tr_dop_adidas_pay/2))-74000)*0.1;$proc_adidas=" (10%)";$pr_adidas=0.10;
}
if((int)$total_info>=100000){
$zarp=((int)$total+((int)$total_tr/2)+((int)$total_tr_dop/2)-((int)$total_info_adidas_pay+((int)$total_info_tr_adidas_pay/2)+((int)$total_info_tr_dop_adidas_pay/2)))*0.15;$proc=" (15%)";$pr=0.15;
$zarp_adidas=((int)$total_info_adidas_pay+((int)$total_info_tr_adidas_pay/2)+((int)$total_info_tr_dop_adidas_pay/2)-74000)*0.1;$proc_adidas=" (10%)";$pr_adidas=0.10;
}
...
$zarp_last1=((int)$total_last1+((int)$tr_last1/2)+((int)$tr_dop_last1/2)-((int)$total_info_adidas_pay_last1+((int)$total_info_tr_adidas_pay_last1/2)+((int)$total_info_tr_dop_adidas_pay_last1/2)))*0.1;$proc_last1=" (10%)";
$zarp_adidas_last1=((int)$total_info_adidas_pay_last1+((int)$total_info_tr_adidas_pay_last1/2)+((int)$total_info_tr_dop_adidas_pay_last1/2))*0.1;$proc_adidas=" (10%)";$pr_adidas=0.10;
}
if((int)$total_all_last1>=100000){
$zarp_last1=((int)$total_last1+((int)$tr_last1/2)+((int)$tr_dop_last1/2)-((int)$total_info_adidas_pay_last1+((int)$total_info_tr_adidas_pay_last1/2)+((int)$total_info_tr_dop_adidas_pay_last1/2)))*0.15;$proc_last1=" (15%)";
$zarp_adidas_last1=((int)$total_info_adidas_pay_last1+((int)$total_info_tr_adidas_pay_last1/2)+((int)$total_info_tr_dop_adidas_pay_last1/2))*0.1;$proc_adidas=" (10%)";$pr_adidas=0.10;
}
} else {
...
$zarp_last2=((int)$total_last2+((int)$tr_last2/2)+((int)$tr_dop_last2/2)-(int)$total_info_adidas_pay_last2-((int)$total_info_tr_adidas_pay_last2/2)+((int)$total_info_tr_dop_adidas_pay_last2/2))*0.1;$proc_last2=" (10%)";
$zarp_adidas_last2=((int)$total_info_adidas_pay_last2+((int)$total_info_tr_adidas_pay_last2/2)+((int)$total_info_tr_dop_adidas_pay_last2/2))*0.1;$proc_adidas=" (10%)";$pr_adidas=0.10;
}
if((int)$total_all_last2>=100000){
$zarp_last2=((int)$total_last2+((int)$tr_last2/2)+((int)$tr_dop_last2/2)-(int)$total_info_adidas_pay_last2-((int)$total_info_tr_adidas_pay_last2/2)+((int)$total_info_tr_dop_adidas_pay_last2/2))*0.15;$proc_last2=" (15%)";
$zarp_adidas_last2=((int)$total_info_adidas_pay_last2+((int)$total_info_tr_adidas_pay_last2/2)+((int)$total_info_tr_dop_adidas_pay_last2/2))*0.1;$proc_adidas=" (10%)";$pr_adidas=0.10;
...
if($row['motive']==4){
$zarp=(int)$total_tr_dop*0.12;$proc=" (12%)";
$zarp_last1=(int)$tr_dop_last1*0.12;$proc_last1=" (12%)";
$zarp_last2=(int)$tr_dop_last2*0.12;$proc_last2=" (12%)";
}
...
if (!empty($tr_order))
{foreach ($tr_order as $index=>$value) {if($value!=0)$output.= $index." (".$value." р - п.), "; else $output_d.= $index." (".$value." р - п.), ";}}


if($row['motive']==4)
{
if (!empty($tr_dop_order))
{foreach ($tr_dop_order as $index=>$value) {if($value!=0)$output.= $index." (".$value." р - п.п.), "; else $output_d.= $index." (".($value)." р - п.п.), ";}}
...
{foreach ($tr_order_way as $index=>$value) {if($value!=0)$output.= $index."* (".$value." р - н.), "; else $output_d.= $index."* (".$value." р - н.), ";}}

if (!empty($tr_order_way_last1))
{foreach ($tr_order_way_last1 as $index=>$value) {if($value!=0)$output.= "!".$index."* (".$value." р - н.), ";} }

if (!empty($tr_order_way_last2))
{foreach ($tr_order_way_last2 as $index=>$value) {if($value!=0)$output.= "!!".$index."* (".$value." р - н.), ";} }
...
if((int)$manager==9){
$aSheet->setCellValue('E9',"Адидас");
$aSheet->getStyle('E9')->applyFromArray($boldFont)->applyFromArray($center);
$aSheet->setCellValue('E10',$zarp_adidas);
$aSheet->getStyle('E10')->applyFromArray($boldFont);
$aSheet->setCellValue('F10'," руб.".$proc_adidas);
}
...
if((int)$manager==9){
$aSheet->setCellValue('E11',(int)$zarp_adidas_last1);
$aSheet->getStyle('E11')->applyFromArray($boldFont);
$aSheet->setCellValue('F11'," руб.".$proc_adidas);
}
...
if((int)$manager==9){
$aSheet->setCellValue('E12',(int)$zarp_adidas_last2);
$aSheet->getStyle('E12')->applyFromArray($boldFont);
$aSheet->setCellValue('F12'," руб.".$proc_adidas);
}

if((int)$manager==9){
$aSheet->setCellValue('H16',"Адидас: ");
$aSheet->getStyle('H16')->applyFromArray($boldFont);
$aSheet->setCellValue('H14',"Расходы на аренду склада: 74000 руб.");
$aSheet->getStyle('H14')->applyFromArray($boldFont)->applyFromArray($right);


$aSheet->setCellValue('H17',$total_info_adidas+$total_info_tr_adidas+$total_info_tr_dop_adidas);
$aSheet->setCellValue('H18',(int)$total_info_adidas_pay+(int)$total_info_tr_adidas_pay/2+(int)$total_info_tr_dop_adidas_pay/2);
$aSheet->getStyle('H17')->applyFromArray($boldFont);
$aSheet->getStyle('H18')->applyFromArray($boldFont);
$aSheet->setCellValue('I17'," руб.");
$aSheet->setCellValue('I18'," руб.");
}
...
if($row['motive']==1||$row['motive']==4){$aSheet->setCellValue('F18',((int)$total+((int)$total_tr/2)+((int)$total_tr_dop/2)));}
if($row['motive']==3){$aSheet->setCellValue('F18',((int)$total+((int)$total_tr_dop/2)+$total_tr_way));}
...
$query_tr_info= "SELECT `id`,`cl_cash`,`cl_minus`,`cl_plus`,`cl_nds`,`tr_cash`,`tr_minus`,`tr_plus`,`tr_nds`,`client` FROM `orders` WHERE `delete`='0' AND `manager`='".$manager."' AND `tr_manager`='".$manager."' AND DATE(`data`) BETWEEN '".mysql_escape_string($date_start_last_info)."' AND '".mysql_escape_string($date_end_last_info)."' AND `client` not in (16,76)";
...
while($pay_tr = mysql_fetch_row($result_pays_tr_info)) {$pays_tr=(int)$pays_tr+(int)$pay_tr[0];}
...
if($zbill[2]==3){if($zbill[4]!=0)$avans=(int)$avans+$zbill[4];  }

if($zbill[2]==2){if($zbill[4]!=0)$premia=(int)$premia+$zbill[4]; }

if($zbill[2]==4){if($zbill[4]!=0)$na_karty=(int)$na_karty+$zbill[4]; }
