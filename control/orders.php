...
//Для бухгалтеров
if($_SESSION["group"]!=5){$navigation="<a class=\"button\" id=\"btnDocs\" href=\"#\" onclick=\"window.location.href='docs.php?doc_id=".$docs[0]."'\">Документы</a><br><a class=\"button\" id=\"btnEdit\" href=\"javascript:;\" onclick=\"$('#fa').load('theme/forms/add_order.php?order=".$row['id']."');$('#fa').dialog({ title: 'Редактировать заявку №".$row['id']."' },{width: 970,height: 820,modal: true,resizable: false});return false\"><b>Редактировать</b></a><br><a  class=\"button\" id=\"btnPrint\" href=\"javascript:;\" onclick=\"$('#dialogpr').dialog({ buttons: [{text: 'Клиенту',click: function() {window.open('control/print.php?mode=cl&id=".base64_encode($row['id'])."');\$(this).dialog('close');}}".$pr_vtl."] },{ resizable: false });\">Распечатать</a><br><a  class=\"button\" id=\"btnPrint_dov\" href=\"javascript:;\" onclick=\"window.open('control/print_dov.php?mode=dov&id=".base64_encode($row['id'])."');\">Доверенность</a><hr style=\"width: 100%; height: 2px;\" /><br><a class=\"button\" id=\"btnDelete\" href=\"javascript:;\" onclick=\"$('#result').html('Удалить выбранную заявку?');$('#result').dialog({ title: 'Внимание' },{ modal: true },{ resizable: false },{ buttons: [{text: 'Да',click: function() {\$.post('control/orders.php?mode=delete&id=".$row['id']."', function(data) {jQuery('#table').trigger('reloadGrid');$('#result').dialog('close');$('#result_temp').html(data);$('#result_temp').dialog({ title: 'Внимание' },{ height: 90 },{ width: 400 },{ modal: true },{ resizable: false });});}},{text: 'Нет',click: function() {\$(this).dialog('close');}}] });\" style=\"background: #ff1212;\">Удалить</a><br>";
...
echo '<font color="red" size="3"><div alegn="center">Невозможно удалить! В заявке имеются платежи...</font> Обратитесь к директору...</div>';
}
	}
